...
(function() {
  ...
  async function buildStockTwits(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:#94a3b8;margin-bottom:8px">💬 StockTwits 社群情緒（美股 / 加密）— 全球散戶討論熱度</div>' +
      '<div style="display:flex;gap:6px;margin-bottom:10px"><input id="st-sym" value="TSLA" placeholder="美股代號 (例: AAPL, NVDA, TSLA, BTC.X, ETH.X)" style="flex:1;padding:6px;background:#0f172a;border:1px solid #334155;color:#fff;border-radius:4px">' +
      '<button id="st-go" style="padding:6px 14px;background:#2563eb;border:0;color:#fff;border-radius:4px;cursor:pointer">查情緒</button></div>' +
      '<div id="st-result"></div></div>';
    $('st-go').addEventListener('click', async () => {
      const sym = $('st-sym').value.trim().toUpperCase();
      if (!sym) return;
      $('st-result').innerHTML = '<div style="color:#94a3b8;padding:10px">⏳ 抓 StockTwits 社群討論中...</div>';
      try {
        ...
        const color = score === 'BULLISH' ? '#34d399' : score === 'BEARISH' ? '#f87171' : '#94a3b8';
        let html = '<div style="background:rgba(96,165,250,0.05);border:1px solid rgba(96,165,250,0.3);border-radius:6px;padding:10px;margin-bottom:10px">' +
          ...
          '<div style="display:flex;height:24px;border-radius:4px;overflow:hidden;background:#1e293b">' +
          '<div style="background:#34d399;width:' + bullPct + '%" title="多方 ' + bull + '"></div>' +
          '<div style="background:#94a3b8;width:' + ((neutral/total)*100) + '%" title="中性 ' + neutral + '"></div>' +
          ...
          '<span style="color:#94a3b8">⚪ 中 ' + neutral + '</span>' +
          '<span style="color:#f87171">🐻 空 ' + bear + ' (' + bearPct.toFixed(0) + '%)</span></div>' +
          '<div style="margin-top:8px;font-size:11px;color:#94a3b8">樣本: 最新 ' + total + ' 則留言</div></div>';

        // Top messages
        html += '<div style="font-size:13px;color:#94a3b8;margin-bottom:6px">📝 最新 5 則熱門留言</div>';
        msgs.slice(0, 5).forEach(m => {
          const sent = m.entities?.sentiment?.basic;
          const sentColor = sent === 'Bullish' ? '#34d399' : sent === 'Bearish' ? '#f87171' : '#94a3b8';
          const sentIcon = sent === 'Bullish' ? '🐂' : sent === 'Bearish' ? '🐻' : '⚪';
          const text = (m.body || '').slice(0, 200);
          const time = new Date(m.created_at).toLocaleString();
          const user = m.user?.username || 'anon';
          html += '<div style="background:#0a0f1c;border:1px solid #1e293b;border-left:3px solid ' + sentColor + ';border-radius:4px;padding:8px;margin-bottom:6px;font-size:12px">' +
  ...
  async function buildOptionChain(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:#94a3b8;margin-bottom:8px">📋 期權鏈（美股）— 透過 Worker proxy 抓 Yahoo Finance options</div>' +
      '<div style="display:flex;gap:6px;margin-bottom:10px"><input id="oc-sym" value="AAPL" placeholder="美股代號 (例: AAPL, TSLA, NVDA)" style="flex:1;padding:6px;background:#0f172a;border:1px solid #334155;color:#fff;border-radius:4px">' +
    ...
    $('oc-go').addEventListener('click', async () => {
      ...
      if (!sym) return;
      $('oc-result').innerHTML = '<div style="color:#94a3b8;padding:10px">⏳ 抓期權鏈資料...</div>';
      try {
        ...
        let html = '<div style="display:flex;justify-content:space-between;margin-bottom:8px;font-size:13px"><span style="color:#94a3b8">標的價: <strong style="color:#fff">$' + underlying + '</strong></span><span style="color:#94a3b8">到期日: <strong style="color:#fff">' + expiry + '</strong></span><span style="color:#94a3b8">合約數: <strong style="color:#fff">' + (j.calls.length + j.puts.length) + '</strong></span></div>';
        html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">';
        // Calls
        html += '<div><div style="font-weight:700;color:#34d399;margin-bottom:4px">📈 Calls (買權)</div>' +
          '<table style="width:100%;border-collapse:collapse;font-size:11px"><thead><tr style="border-bottom:1px solid #334155;color:#94a3b8"><th style="padding:4px;text-align:right">Strike</th><th style="text-align:right">Last</th><th style="text-align:right">Bid</th><th style="text-align:right">Ask</th><th style="text-align:right">OI</th></tr></thead><tbody>';
        j.calls.slice(0, 15).forEach(c => {
          const itm = c.strike < underlying;
          html += '<tr style="border-bottom:1px solid #1e293b' + (itm ? ';background:rgba(52,211,153,0.05)' : '') + '">' +
        ...
        j.puts.slice(0, 15).forEach(p => {
          ...
          html += '<tr style="border-bottom:1px solid #1e293b' + (itm ? ';background:rgba(248,113,113,0.05)' : '') + '">' +
        ...
        html += '<div style="margin-top:10px;padding:8px;background:rgba(96,165,250,0.05);border:1px solid rgba(96,165,250,0.2);border-radius:4px;font-size:11px;color:#94a3b8">💡 ITM (價內) 行權價以淡綠/紅底標示。OI = 未平倉量，反映市場關注度。</div>';
  ...
  async function buildEarningsAI(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:#94a3b8;margin-bottom:8px">📊 Earnings Call AI 摘要（美股）— Llama 3.3 70B 解讀 SEC 8-K filing</div>' +
      '<div style="display:flex;gap:6px;margin-bottom:10px"><input id="ea-sym" value="AAPL" placeholder="美股代號 (例: AAPL, NVDA, TSLA)" style="flex:1;padding:6px;background:#0f172a;border:1px solid #334155;color:#fff;border-radius:4px">' +
    ...
    $('ea-go').addEventListener('click', async () => {
      ...
      if (!sym) return;
      $('ea-result').innerHTML = '<div style="color:#94a3b8;padding:10px">⏳ 抓最新 8-K filing 並做 AI 摘要（~15 秒）...</div>';
      try {
        ...
        let html = '<div style="background:rgba(220,38,38,0.05);border:1px solid rgba(220,38,38,0.3);border-radius:6px;padding:12px;margin-bottom:10px">' +
          '<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:8px"><span style="color:#94a3b8">' + sym + ' 最新 8-K</span><span style="color:#94a3b8">' + filingDate + '</span></div>' +
