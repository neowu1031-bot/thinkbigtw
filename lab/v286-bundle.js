...
(function () {
  ...
  function V286_inject(parentTabId, id, title, builder) {
    ...
    const sec = V286_el(`<section id="${id}" style="margin:24px 0;padding:20px;background:var(--card-bg,#1a1a2e);border-radius:12px;border:1px solid var(--border,#2a2a4a)">
  ...
  function V286_buildLongShort(containerId) {
    ...
    c.innerHTML = `
      ...
      <div style="background:#0f0f1e;padding:16px;border-radius:8px;margin-bottom:12px">
        <textarea id="ls286-pool" rows="3" placeholder="股票代碼，逗號分隔（例：AAPL, MSFT, GOOG, META, NVDA, TSLA, AMD, INTC）" style="width:100%;padding:10px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px"></textarea>
        <div style="margin-top:8px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <label style="color:#aaa;font-size:13px">配對數:
            <input id="ls286-pairs" type="number" min="1" max="5" value="3" style="width:60px;padding:6px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-left:4px"></label>
    ...
    V286_$('ls286-run').onclick = async () => {
      ...
      try {
        ...
        r.innerHTML = `
          ...
          <div style="margin-top:16px;padding:12px;background:#0f0f1e;border-radius:6px;font-size:12px;color:#aaa">
            <b>💡 策略說明：</b><br>
      ...
      <div style="background:#0f0f1e;padding:16px;border-radius:8px;margin-bottom:12px">
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px">
          <label style="color:#aaa">📉 Value 價值
            <select id="fi286-value" style="width:100%;padding:8px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-top:4px">
              <option value="0">不偏重</option><option value="1">偏重</option><option value="2" selected>強烈偏重</option>
            </select></label>
          <label style="color:#aaa">📈 Growth 成長
            <select id="fi286-growth" style="width:100%;padding:8px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-top:4px">
              <option value="0">不偏重</option><option value="1" selected>偏重</option><option value="2">強烈偏重</option>
            </select></label>
          <label style="color:#aaa">🚀 Momentum 動能
            <select id="fi286-momentum" style="width:100%;padding:8px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-top:4px">
              <option value="0">不偏重</option><option value="1" selected>偏重</option><option value="2">強烈偏重</option>
            </select></label>
          <label style="color:#aaa">💎 Quality 品質
            <select id="fi286-quality" style="width:100%;padding:8px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-top:4px">
              <option value="0">不偏重</option><option value="1">偏重</option><option value="2" selected>強烈偏重</option>
            </select></label>
        </div>
  ...
  function V286_buildRiskParity(containerId) {
    ...
    c.innerHTML = `
      ...
      <div style="background:#0f0f1e;padding:16px;border-radius:8px;margin-bottom:12px">
        <input id="rp286-symbols" placeholder="股票代碼，逗號分隔（例：SPY, TLT, GLD, DBC, VWO）" style="width:100%;padding:10px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px">
    ...
    V286_$('rp286-run').onclick = async () => {
      ...
      try {
        ...
        // 圓餅圖
        let cum = 0;
        const colors = ['#4ade80', '#3b82f6', '#fbbf24', '#ef4444', '#a855f7', '#ec4899', '#14b8a6', '#f97316'];
        ...
        r.innerHTML = `
          <div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap">
            ...
            <table style="flex:1;font-size:13px;border-collapse:collapse;min-width:300px">
              <tr style="background:#2a2a4a"><th style="padding:6px;text-align:left">資產</th><th style="padding:6px">權重</th><th style="padding:6px">波動度</th><th style="padding:6px">風險貢獻</th></tr>
      ...
      <div style="background:#0f0f1e;padding:16px;border-radius:8px;margin-bottom:12px">
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px">
          <label style="color:#aaa">勝率 % (歷史測試或主觀估計)
            <input id="k286-win" type="number" min="1" max="99" value="55" style="width:100%;padding:8px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-top:4px"></label>
          <label style="color:#aaa">平均勝報酬 % (停利目標)
            <input id="k286-gain" type="number" min="0.1" step="0.1" value="15" style="width:100%;padding:8px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-top:4px"></label>
          <label style="color:#aaa">平均敗報酬 % (停損)
            <input id="k286-loss" type="number" min="0.1" step="0.1" value="5" style="width:100%;padding:8px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-top:4px"></label>
          <label style="color:#aaa">總資金 (NTD)
            <input id="k286-capital" type="number" min="1000" value="1000000" style="width:100%;padding:8px;background:#2a2a4a;border:1px solid #444;color:#fff;border-radius:4px;margin-top:4px"></label>
        </div>
      ...
      r.innerHTML = `
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px">
          ...
          <div style="background:#0f3a1f;padding:16px;border-radius:8px">
            ...
            <div style="color:#3b82f6;font-size:32px;font-weight:bold">${V286_fmt(quarterKelly * 100, 2)}%</div>
        ...
        <div style="margin-top:16px;padding:12px;background:#0f0f1e;border-radius:6px;font-size:13px">
