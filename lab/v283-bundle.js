...
(function() {
  ...
  async function buildTPEx(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">🇹🇼 TPEx 櫃買中心 — 台股 OTC 市場日報（即時抓取最新交易日資料）</div>' +
      ...
      '<select id="tpex-type" style="padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px"><option value="otc">櫃買加權</option><option value="emerging">興櫃</option><option value="ranking">櫃買漲幅榜</option></select></div>' +
      '<div id="tpex-result"></div></div>';

    $('tpex-go').addEventListener('click', async () => {
      const type = $('tpex-type').value;
      $('tpex-result').innerHTML = '<div style="color:var(--text-secondary);padding:10px">⏳ 抓取 TPEx 資料中...</div>';
      try {
        ...
        if (type === 'otc') {
          ...
          if (j.index) {
            html += '<div style="background:var(--bg-base);padding:8px;border-radius:4px"><div style="color:var(--text-secondary);font-size:11px">收盤</div><div style="font-size:18px;font-weight:700">' + fmt(j.index.close) + '</div></div>';
            html += '<div style="background:var(--bg-base);padding:8px;border-radius:4px"><div style="color:var(--text-secondary);font-size:11px">漲跌</div><div style="font-size:16px;font-weight:700;color:' + (j.index.change >= 0 ? '#34d399' : '#f87171') + '">' + (j.index.change >= 0 ? '+' : '') + fmt(j.index.change) + ' (' + (j.index.change_pct >= 0 ? '+' : '') + fmt(j.index.change_pct, 2) + '%)</div></div>';
            html += '<div style="background:var(--bg-base);padding:8px;border-radius:4px"><div style="color:var(--text-secondary);font-size:11px">成交額</div><div style="font-size:16px;font-weight:700">' + fmtBig(j.index.amount) + '</div></div>';
          }
          html += '</div>';
        } else if (j.list) {
          html += '<div style="font-size:14px;color:#fff;margin-bottom:8px">' + (type === 'ranking' ? '🚀 櫃買漲幅榜 Top 30' : '🌱 興櫃熱門 Top 30') + '</div>';
          html += '<table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="border-bottom:1px solid var(--border-strong);color:var(--text-secondary)"><th style="padding:6px;text-align:left">代號</th><th style="text-align:left">名稱</th><th style="text-align:right">收盤</th><th style="text-align:right">漲跌</th><th style="text-align:right">漲幅%</th><th style="text-align:right">成交</th></tr></thead><tbody>';
          j.list.slice(0, 30).forEach(s => {
            html += '<tr style="border-bottom:1px solid var(--bg-elevated)"><td style="padding:6px;font-weight:700">' + s.symbol + '</td><td>' + (s.name || '-') + '</td><td style="text-align:right">' + fmt(s.close, 2) + '</td><td style="text-align:right;color:' + (s.change >= 0 ? '#34d399' : '#f87171') + '">' + (s.change >= 0 ? '+' : '') + fmt(s.change, 2) + '</td><td style="text-align:right;color:' + (s.change_pct >= 0 ? '#34d399' : '#f87171') + ';font-weight:700">' + (s.change_pct >= 0 ? '+' : '') + fmt(s.change_pct, 2) + '%</td><td style="text-align:right">' + fmtBig(s.volume) + '</td></tr>';
          });
          html += '</tbody></table>';
        } else {
          html += '<div style="color:var(--text-secondary)">⚠️ 無資料</div>';
        }
        html += '<div style="margin-top:8px;font-size:11px;color:var(--text-muted)">資料源: TPEx 證券櫃檯買賣中心 OpenData · ' + (j.date || '最新交易日') + '</div></div>';
  ...
  async function buildTaifex(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">📊 期交所 TAIFEX — 台指期 / 選擇權 / 三大法人期貨持倉每日統計</div>' +
      ...
      '<select id="tx-type" style="padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px"><option value="futures">📈 期貨成交統計</option><option value="institutional">🏢 三大法人期貨持倉</option><option value="options">📋 選擇權統計</option></select></div>' +
      '<div id="tx-result"></div></div>';

    $('tx-go').addEventListener('click', async () => {
      const type = $('tx-type').value;
      $('tx-result').innerHTML = '<div style="color:var(--text-secondary);padding:10px">⏳ 抓 TAIFEX...</div>';
      try {
        ...
        if (j.rows && j.rows.length) {
          ...
          if (j.headers) {
            html += '<thead><tr style="border-bottom:1px solid var(--border-strong);color:var(--text-secondary)">';
            j.headers.forEach(h => { html += '<th style="padding:6px;text-align:right">' + h + '</th>'; });
            html += '</tr></thead>';
          }
          html += '<tbody>';
          j.rows.slice(0, 50).forEach((row, idx) => {
            html += '<tr style="border-bottom:1px solid var(--bg-elevated)">';
            row.forEach((cell, ci) => {
              const isFirst = ci === 0;
        ...
        html += '<div style="margin-top:8px;font-size:11px;color:var(--text-muted)">資料源: 台灣期貨交易所 OpenData · ' + (j.date || '最新交易日') + '</div></div>';
  ...
  async function buildMOPS(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">📰 公開資訊觀測站 MOPS — 台股上市櫃公司即時重大訊息（法定揭露）</div>' +
      '<div style="display:flex;gap:6px;margin-bottom:10px"><input id="mops-sym" placeholder="代號（空白=全市場）" style="flex:1;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px">' +
    ...
    $('mops-go').addEventListener('click', async () => {
      ...
      $('mops-result').innerHTML = '<div style="color:var(--text-secondary);padding:10px">⏳ 抓 MOPS 重大訊息...</div>';
      try {
        ...
        if (!list.length) { $('mops-result').innerHTML = '<div style="padding:10px;color:var(--text-secondary)">⚠️ 無公告</div>'; return; }
        ...
        list.slice(0, 30).forEach(a => {
          html += '<div style="border-bottom:1px solid var(--bg-elevated);padding:8px 0;font-size:12px">' +
            '<div style="display:flex;justify-content:space-between;margin-bottom:4px">' +
            '<span style="color:#fbbf24;font-weight:700">' + (a.symbol || '-') + ' ' + (a.name || '') + '</span>' +
            '<span style="color:var(--text-secondary);font-size:11px">' + (a.date || '') + ' ' + (a.time || '') + '</span></div>' +
            '<div style="color:#cbd5e1;line-height:1.5">' + (a.title || '').replace(/</g, '&lt;') + '</div>' +
            (a.content ? '<div style="color:var(--text-secondary);font-size:11px;margin-top:4px;line-height:1.4">' + (a.content || '').slice(0, 200).replace(/</g, '&lt;') + '...</div>' : '') +
            '</div>';
        });
        html += '<div style="margin-top:8px;font-size:11px;color:var(--text-muted)">資料源: 公開資訊觀測站 MOPS · 法定揭露每日更新</div></div>';
  ...
  function buildTutorial() {
    ...
    function open() {
      ...
      overlay.style.cssText = 'position:fixed;bottom:80px;left:20px;z-index:99999;background:var(--bg-base);border:1px solid var(--border-strong);border-radius:12px;padding:18px;width:380px;box-shadow:0 24px 60px rgba(0,0,0,0.5)';
      overlay.innerHTML = '<div style="display:flex;justify-content:space-between;margin-bottom:8px"><span class="tut-title" style="font-weight:700;color:#fff;font-size:16px"></span><button class="tut-close" style="background:transparent;border:0;color:var(--text-secondary);cursor:pointer;font-size:18px">×</button></div>' +
        '<div class="tut-body" style="color:#cbd5e1;line-height:1.7;font-size:13px;margin-bottom:14px"></div>' +
        '<div style="display:flex;justify-content:space-between;align-items:center"><span class="tut-progress" style="color:var(--text-muted);font-size:11px"></span>' +
        '<div><button class="tut-prev" style="padding:6px 12px;background:var(--bg-elevated);border:0;color:#cbd5e1;border-radius:4px;cursor:pointer;margin-right:6px">← 上一步</button>' +
  ...
  function buildTaxCalculator(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">💸 台股完整稅費計算器 — 手續費 + 證交稅 + 二代健保 + 股利所得</div>' +
      '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:8px;margin-bottom:10px;font-size:12px">' +
      '<label>類型<select id="tx-mode" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"><option value="trade">📈 買賣交易</option><option value="dividend">💰 股利收入</option></select></label>' +
      '<label id="tx-buy-lbl">買進金額<input type="number" id="tx-buy" value="100000" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
      '<label id="tx-sell-lbl">賣出金額<input type="number" id="tx-sell" value="120000" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
      '<label>券商手續費折扣 %<input type="number" id="tx-disc" value="65" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
      '<label id="tx-div-lbl" style="display:none">股利金額<input type="number" id="tx-div" value="50000" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
      '<label id="tx-bracket-lbl" style="display:none">綜所稅率 %<select id="tx-bracket" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"><option value="5">5% (54 萬以下)</option><option value="12">12% (54-121 萬)</option><option value="20" selected>20% (121-242 萬)</option><option value="30">30% (242-453 萬)</option><option value="40">40% (453+ 萬)</option></select></label>' +
    ...
    $('tx-go').addEventListener('click', () => {
      ...
      if (mode === 'trade') {
        ...
        $('tx-result').innerHTML = '<div style="background:rgba(220,38,38,0.05);border:1px solid rgba(220,38,38,0.3);border-radius:6px;padding:14px">' +
          ...
          '<tr><td style="padding:4px;color:var(--text-secondary)">買進金額</td><td style="text-align:right">$' + buy.toLocaleString() + '</td></tr>' +
          '<tr><td style="padding:4px;color:var(--text-secondary)">買進手續費 (0.1425% × ' + (disc * 100) + '%)</td><td style="text-align:right;color:#f87171">-$' + buyFee.toLocaleString() + '</td></tr>' +
          '<tr><td style="padding:4px;color:var(--text-secondary)">賣出金額</td><td style="text-align:right">$' + sell.toLocaleString() + '</td></tr>' +
          '<tr><td style="padding:4px;color:var(--text-secondary)">賣出手續費</td><td style="text-align:right;color:#f87171">-$' + sellFee.toLocaleString() + '</td></tr>' +
          '<tr><td style="padding:4px;color:var(--text-secondary)">證交稅 (0.3%)</td><td style="text-align:right;color:#f87171">-$' + tax.toLocaleString() + '</td></tr>' +
          '<tr style="border-top:1px solid var(--border-strong)"><td style="padding:6px;color:#fbbf24;font-weight:700">總費用</td><td style="text-align:right;color:#fbbf24;font-weight:700">$' + totalFee.toLocaleString() + '</td></tr>' +
          ...
          '</tbody></table>' +
          '<div style="margin-top:10px;padding:8px;background:rgba(96,165,250,0.05);border-radius:4px;font-size:11px;color:var(--text-secondary);line-height:1.6">' +
        ...
        $('tx-result').innerHTML = '<div style="background:rgba(220,38,38,0.05);border:1px solid rgba(220,38,38,0.3);border-radius:6px;padding:14px">' +
          ...
          '<table style="width:100%;border-collapse:collapse;font-size:13px"><tbody>' +
          '<tr><td style="padding:4px;color:var(--text-secondary)">股利收入</td><td style="text-align:right">$' + div.toLocaleString() + '</td></tr>' +
          '<tr><td style="padding:4px;color:var(--text-secondary)">二代健保補充保費 (' + (div >= 20000 ? '2.11%' : '免徵') + ')</td><td style="text-align:right;color:#f87171">-$' + nhi.toLocaleString() + '</td></tr>' +
          '<tr><td style="padding:4px;color:var(--text-secondary)">綜合所得稅 (' + (bracket * 100) + '% 級距)</td><td style="text-align:right;color:#f87171">-$' + incomeTax.toLocaleString() + '</td></tr>' +
          '<tr><td style="padding:4px;color:var(--text-secondary)">股利可抵減稅額 (8.5%)</td><td style="text-align:right;color:#34d399">+$' + Math.min(dividendCredit, 80000).toLocaleString() + '</td></tr>' +
          '<tr style="border-top:1px solid var(--border-strong)"><td style="padding:6px;color:#fbbf24;font-weight:700">應繳稅額</td><td style="text-align:right;color:#fbbf24;font-weight:700">$' + (finalTax + nhi).toLocaleString() + '</td></tr>' +
  ...
  async function buildCoinGecko(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">🪙 加密前 100 + 全球熱搜 — CoinGecko 開放 API（無需付費）</div>' +
      ...
      '<select id="cg-sort" style="padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px"><option value="market_cap">市值排行</option><option value="volume">成交量</option><option value="gain">24h 漲幅</option><option value="loss">24h 跌幅</option></select></div>' +
    ...
    $('cg-go').addEventListener('click', async () => {
      ...
      try {
        ...
        let html = '<div style="background:rgba(251,191,36,0.05);border:1px solid rgba(251,191,36,0.3);border-radius:6px;padding:12px"><div style="font-size:14px;color:#fff;font-weight:700;margin-bottom:8px">🪙 加密前 ' + data.length + ' 大幣種</div>' +
          '<table style="width:100%;border-collapse:collapse;font-size:11px"><thead><tr style="border-bottom:1px solid var(--border-strong);color:var(--text-secondary)">' +
        ...
        data.slice(0, 50).forEach((coin, i) => {
          ...
          html += '<tr style="border-bottom:1px solid var(--bg-elevated)">' +
            ...
            '<td><img src="' + coin.image + '" width="14" height="14" style="vertical-align:middle;margin-right:4px"> <strong>' + coin.symbol.toUpperCase() + '</strong> <span style="color:var(--text-secondary);font-size:10px">' + coin.name + '</span></td>' +
  ...
  function buildEfficientFrontier(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">📐 Markowitz 效率前緣 — 投組最佳化（給定風險的最高報酬 / 給定報酬的最低風險）</div>' +
      '<div style="display:flex;gap:6px;margin-bottom:10px"><input id="ef-syms" placeholder="3-5 檔代號（如 2330,2454,2308,2882,1101）" value="2330,2454,2308,2882" style="flex:1;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px">' +
    ...
    $('ef-go').addEventListener('click', async () => {
      ...
      try {
        ...
        let svg = '<svg width="100%" height="' + H + '" viewBox="0 0 ' + W + ' ' + H + '" style="background:var(--bg-base);border-radius:6px;margin-top:8px">';
        portfolios.forEach(p => {
          const x = ((p.std - xMin) / (xMax - xMin)) * (W - 60) + 50;
          const y = H - ((p.ret - yMin) / (yMax - yMin)) * (H - 60) - 30;
          const c = p.sharpe > 0.8 ? '#34d399' : p.sharpe > 0.4 ? '#fbbf24' : 'var(--text-muted)';
        ...
        svg += '<circle cx="' + mvX.toFixed(0) + '" cy="' + mvY.toFixed(0) + '" r="8" fill="var(--accent)" stroke="#fff" stroke-width="2"/>';
        svg += '<text x="' + (mvX + 12) + '" y="' + mvY + '" fill="#fff" font-size="11">🛡️ Min Variance</text>';
        // Axes labels
        svg += '<text x="' + (W / 2) + '" y="' + (H - 5) + '" text-anchor="middle" fill="var(--text-secondary)" font-size="11">風險 (年化波動率)</text>';
        svg += '<text x="10" y="' + (H / 2) + '" fill="var(--text-secondary)" font-size="11" transform="rotate(-90 10,' + (H / 2) + ')">年化期望報酬</text>';
        svg += '</svg>';

        let html = svg;
        html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px">';
        [{ label: '⭐ Max Sharpe (最高夏普)', p: maxSharpe, color: '#dc2626' }, { label: '🛡️ Min Variance (最低風險)', p: minVar, color: 'var(--accent)' }].forEach(c => {
          html += '<div style="background:rgba(' + (c.color === '#dc2626' ? '220,38,38' : '37,99,235') + ',0.05);border:1px solid ' + c.color + ';border-radius:6px;padding:10px">' +
            ...
            '<div style="margin-top:8px;font-size:11px;color:var(--text-secondary)">配置: ' + syms.map((s, i) => s + ' ' + (c.p.w[i] * 100).toFixed(1) + '%').join(' / ') + '</div></div>';
        });
        html += '</div>';
        html += '<div style="margin-top:10px;padding:8px;background:rgba(124,58,237,0.05);border-radius:4px;font-size:11px;color:var(--text-secondary);line-height:1.6">💡 5000 個隨機投組模擬。色點顏色越綠 Sharpe 越高。⭐ 紅點 = Max Sharpe，最佳風險調整後報酬。🛡️ 藍點 = Min Variance，最穩健配置。</div>';
