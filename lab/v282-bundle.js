...
(function() {
  ...
  function buildLangSwitcher() {
    ...
    menu.style.cssText = 'display:none;position:fixed;top:48px;right:64px;z-index:9999;background:var(--bg-base);border:1px solid var(--border-strong);border-radius:8px;padding:6px;box-shadow:0 12px 30px rgba(0,0,0,0.4);min-width:160px';
    LANGS.forEach(l => {
  ...
  function buildArticleGen(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">📝 AI 觀點文章生成 — Llama 3.3 70B 為任何股票寫 1500 字 Seeking Alpha 級分析</div>' +
      '<div style="display:flex;gap:6px;margin-bottom:10px"><input id="ag-sym" placeholder="代號 (例: 2330, AAPL, NVDA)" value="2330" style="flex:1;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px">' +
      '<select id="ag-style" style="padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px"><option value="bull">🐂 多頭觀點</option><option value="bear">🐻 空頭觀點</option><option value="balanced">⚖️ 平衡分析</option><option value="contrarian">🔄 反向思考</option></select>' +
    ...
    $('ag-go').addEventListener('click', async () => {
      ...
      $('ag-result').innerHTML = '<div style="color:var(--text-secondary);padding:14px">⏳ Llama 70B 撰寫中（~30 秒）...</div>';
      try {
        ...
        $('ag-result').innerHTML = '<article style="background:rgba(220,38,38,0.05);border:1px solid rgba(220,38,38,0.3);border-radius:6px;padding:20px;font-size:14px;line-height:1.8">' +
          '<header style="display:flex;justify-content:space-between;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid var(--border-strong)">' +
          '<h3 style="margin:0;font-size:18px;color:#fff">' + sym + ' — ' + styleMap[style] + '觀點</h3>' +
          '<span style="color:var(--text-secondary);font-size:11px">🤖 Llama 3.3 70B · ' + new Date().toLocaleString() + '</span></header>' +
          '<div style="color:#cbd5e1;white-space:pre-wrap">' + (j.article || '').replace(/</g, '&lt;') + '</div>' +
          '<footer style="margin-top:14px;padding-top:10px;border-top:1px solid var(--border-strong);color:var(--text-muted);font-size:11px">⚠️ 此文由 AI 生成，僅供參考。投資有風險，請自行判斷。' + (j.tokenCount ? ' · ' + j.tokenCount + ' tokens' : '') + '</footer>' +
          '</article>';
      } catch (e) {
  ...
  function buildAnomalyDetector(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">🚨 AI 市場異常偵測 — 每日掃描 2125 檔 / 770k 筆，找突破 / 爆量 / 反轉訊號</div>' +
    ...
    $('ad-go').addEventListener('click', async () => {
      $('ad-result').innerHTML = '<div style="color:var(--text-secondary);padding:10px">⏳ 掃描所有股票...</div>';
      try {
        ...
        let html = '<div style="background:rgba(234,88,12,0.05);border:1px solid rgba(234,88,12,0.3);border-radius:6px;padding:12px"><div style="font-size:16px;font-weight:700;color:#fff;margin-bottom:8px">🚨 偵測到 ' + anomalies.length + ' 個異常事件（顯示 Top 30）</div>' +
          '<table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="border-bottom:1px solid var(--border-strong);color:var(--text-secondary)"><th style="padding:6px;text-align:left">代號</th><th style="text-align:right">收盤</th><th style="text-align:right">漲跌%</th><th style="text-align:left">異常旗標</th><th style="text-align:right">成交</th></tr></thead><tbody>';
        top.forEach(a => {
          html += '<tr style="border-bottom:1px solid var(--bg-elevated);cursor:pointer" data-sym="' + a.sym + '">' +
        ...
        html += '</tbody></table>' +
          '<div style="margin-top:10px;padding:8px;background:rgba(96,165,250,0.05);border-radius:4px;font-size:11px;color:var(--text-secondary)">💡 點擊任何一行可查詢個股深度。掃描範圍: 過去 30 天 vs 今日。</div></div>';
  ...
  function buildMentorChat(containerId) {
    ...
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">💬 投資導師對話 — 6 位世界頂級投資人 AI 角色扮演（Llama 3.3 70B）</div>' +
      '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:6px;margin-bottom:10px">' +
      MENTORS.map(m => '<div class="mt-card" data-id="' + m.id + '" style="background:var(--bg-base);border:1px solid var(--border-strong);border-radius:6px;padding:10px;cursor:pointer;transition:all 0.15s"><div style="font-size:20px">' + m.icon + '</div><div style="font-weight:700;color:#fff;font-size:13px">' + m.name + '</div><div style="color:var(--text-secondary);font-size:11px;margin-top:2px">' + m.desc + '</div></div>').join('') +
      '</div>' +
      '<div id="mt-chat" style="display:none;background:var(--bg-base);border:1px solid var(--border-strong);border-radius:8px;padding:12px;margin-bottom:10px">' +
      '<div id="mt-history" style="max-height:400px;overflow:auto;margin-bottom:10px"></div>' +
      '<div style="display:flex;gap:6px"><input id="mt-input" placeholder="問導師任何投資問題..." style="flex:1;padding:8px;background:var(--bg-base);border:1px solid var(--text-muted);color:#fff;border-radius:4px">' +
      ...
      '<div id="mt-info" style="color:var(--text-secondary);font-size:12px;padding:6px">點選一位導師開始對話</div></div>';
    ...
    c.querySelectorAll('.mt-card').forEach(card => {
      ...
      card.addEventListener('click', () => {
        c.querySelectorAll('.mt-card').forEach(x => x.style.borderColor = 'var(--border-strong)');
  ...
  function buildGoalPlanner(containerId) {
    const c = $(containerId);
    if (!c) return;
    c.innerHTML = '<div style="padding:8px"><div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">🎯 目標導向投資規劃器 — 退休 / 買房 / 子女教育，5 條情境路徑模擬</div>' +
      '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:8px;margin-bottom:10px;font-size:12px">' +
      '<label>目標<select id="gp-goal" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"><option value="退休">🏖️ 退休</option><option value="買房頭款">🏠 買房頭款</option><option value="子女教育">🎓 子女教育</option><option value="財富自由">💰 財富自由</option><option value="緊急金">🛡️ 緊急金</option></select></label>' +
      '<label>目標金額<input type="number" id="gp-target" value="50000000" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
      '<label>目前年齡<input type="number" id="gp-age" value="30" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
      '<label>目標年齡<input type="number" id="gp-target-age" value="60" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
      '<label>目前資產<input type="number" id="gp-cur" value="500000" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
      '<label>每月可投入<input type="number" id="gp-monthly" value="20000" style="width:100%;padding:6px;background:var(--bg-base);border:1px solid var(--border-strong);color:#fff;border-radius:4px;margin-top:4px"></label>' +
    ...
    $('gp-go').addEventListener('click', () => {
      ...
      const SCENARIOS = [
        { name: '保守（債券 + 高股息）', annRet: 0.04, color: 'var(--text-secondary)' },
      ...
      let html = '<div style="background:rgba(22,163,74,0.05);border:1px solid rgba(22,163,74,0.3);border-radius:6px;padding:12px;margin-bottom:10px">' +
        ...
        '<div style="font-size:12px;color:var(--text-secondary)">起點 $' + fmtBig(cur) + ' · 每月 $' + fmtBig(monthly) + ' · 總投入 $' + fmtBig(totalIn) + '</div>' +
        '</div>';
      // SVG visualization
      const W = 800, H = 280;
      ...
      html += svg;
      // Table
      html += '<table style="width:100%;border-collapse:collapse;font-size:12px;margin-top:10px"><thead><tr style="border-bottom:1px solid var(--border-strong);color:var(--text-secondary)">' +
      ...
      html += '</tbody></table>' +
        '<div style="margin-top:10px;padding:8px;background:rgba(96,165,250,0.05);border-radius:4px;font-size:11px;color:var(--text-secondary);line-height:1.6">💡 「需每月」 = 達成目標的最低每月投入。建議：年輕（30+）配置可較積極，接近目標年齡時轉穩健。</div>';
