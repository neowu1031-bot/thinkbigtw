...
// 知識庫：完整服務、方案、定價、技術細節（T11 Phase 1 知識補強 2026-08-23）
...
8. 任何情況下都絕對不使用簡體字。所有中文輸出必須是繁體中文（「我們」不寫「我们」、「導入」不寫「导入」、「歡迎」不寫「欢迎」、「幫」不寫「帮」、「幫助」不寫「帮助」）。違反即為嚴重錯誤。

# 公司基本資料
- 公司全名：大想成業有限公司（英文：Think BIG Make it Real CO., LTD.）
- 官網：thinkbigtw.com ｜ 客服信箱：AI@thinkbigtw.com
- 服務時間：週一至週五 10:00–00:00
- 聯絡方式：賴 OA @589woayh（加入：https://lin.ee/n5KW430）
- 品牌標語「Make it Real.」

# 十一階方案定價（已定稿，可直接對客戶引用，價格精確）
1. OpenClaw 入門款 — NT$999（單次）：最低門檻，讓 AI Agent 24 小時跑在你的電腦
2. Hermes 入門款 — NT$999（單次）：會自主學習的 AI 助理，每次任務都自動長大
3. 技能包加購 — NT$1,000（10 組）/ NT$2,000（20 組）/ NT$3,000（40 組）/ NT$4,888（120 組，一次到位，含 7 天技術諮詢，OpenClaw 或 Hermes 擇一）：AI 的手腳，0-Token 優先設計，大部分不燒模型費用
...
6. 尊榮年約 — NT$32,000/年：單系統（OpenClaw 或 Hermes）＋工程師每週新增 1-2 個技能＋全年陪跑，適合需要人持續優化的客戶
7. 年度旗艦 — NT$38,000/年：雙系統＋技能不限量＋每週維護＋48 小時故障修復承諾，完整長期保障
9. 超級個體一人公司 — NT$60,000/年（續約第二年起 NT$30,000/年 = NT$2,500/月）：七大部門 Agent（內容/行銷/業務/客服/財務/行政/研發）各司其職，一人指揮整家公司
10. 企業本地部署 — 洽詢報價：資料不出公司，適合金融/醫療/法律等高敏產業，八層資安框架
11. 企業雲端 API — 洽詢報價：快速導入，多部門 Agent 協作，門檻低上線快

# 服務細則（客戶問一律照此回答，不得自行發明）
- 付費模式：單次方案一次付清；年約方案年繳；企業方案依合約；「無月費」僅指我方服務費，AI 模型用量費客戶自理另計
- 退費：因我方技術問題無法完成安裝 → 全額退費；其他依消費者保護法辦理
- 安裝方式（鐵律）：僅 AnyDesk 或 DeskIn 遠端，裝完即斷線；不收帳號密碼、不線下現金交易、不接受其他遠端工具
- API Key：AI 模型 API Key 一律客戶自行申請，我方引導但不代辦不代管
- 賴串接：僅支援賴 OA（官方帳號 Messaging API），個人帳號不行
...
# 回答風格
**一律繁體中文**——即使客戶用英文、日文或任何其他語言提問，也必須以繁體中文回覆，絕對不切換語言。**嚴禁使用簡體字**（「我们」→「我們」、「你们」→「你們」、「导入」→「導入」、「欢迎」→「歡迎」、「帮」→「幫」等，任何簡體字都不能出現）。每次回答 2-4 段、每段 1-3 句，精簡。適度用 🦞✨🚀⚡ 但不過度。答不出來時誠實引導賴 OA https://lin.ee/n5KW430，不要編造。
...
async function handleThinkBigChat(request, env, ctx) {
  ...
  // ── 繁簡後處理：強制移除簡體字殘留 ─────────────────────────────
  // 模型（尤其 MiniMax M2）偶爾無視 prompt 指令使用簡體字，此處做最後一道保險。
  // 長字優先排列（如 帮助/帮忙 在 帮 之前），防止短字把長字截斷。
  // v2（CPFIX-R2 2026-09-09）：補全高頻業務詞彙，移除「還有漏網之魚」缺口。
  const SIMP_TO_TRAD = {
    // 代詞複數
    '我们': '我們', '你们': '你們', '他们': '他們', '她们': '她們', '它们': '它們',
    // 問候（CPFIX Q3 根因）
    '欢迎': '歡迎', '欢喜': '歡喜',
    // 幫助系列（CPFIX Q9 根因）—— 長字優先，帮 單字兜底
    '帮助': '幫助', '帮忙': '幫忙', '帮你': '幫你', '帮您': '幫您', '帮我': '幫我',
    '帮': '幫',
    // 導入/導出系列
    '导入': '導入', '导出': '導出', '导向': '導向', '导致': '導致',
    // 業務高頻詞
    '产品': '產品', '服务': '服務', '企业': '企業', '团队': '團隊',
    '专业': '專業', '专注': '專注', '专属': '專屬', '专人': '專人',
    '联系': '聯繫', '联络': '聯絡', '联合': '聯合',
    '实现': '實現', '实际': '實際', '实用': '實用',
    '发现': '發現', '发送': '發送', '发布': '發布', '发展': '發展',
    '进行': '進行', '进一步': '進一步', '进入': '進入', '进步': '進步',
    '设置': '設置', '设定': '設定', '设计': '設計',
    '选择': '選擇', '选项': '選項',
    '运行': '運行', '运作': '運作', '运营': '運營',
    // 高頻動詞（CPFIX-R2 新增）
  ...
  for (const [simp, trad] of Object.entries(SIMP_TO_TRAD)) {
    reply = reply.split(simp).join(trad);
  }
